...
Phase: Intake & Discovery
  ...
  - Draft & confirm page list
      |
      v
Phase: Plan the Page Set
  - Log approved scope + trackers
  - Prep section inventory
      |
      v
Phase: Define Visual System
...
Phase: Build Page Skeletons
  ...
  - Outline pages & slot sections
      |
      v
Phase: Draft Page Copy
  - Rewrite on-page content
  - Update usage trackers
      |
      v
Phase: Site-Wide Assembly
...
- For Horizon builds, bootstrap the Next.js workspace if it does not exist by running `python public-sites/scripts/bootstrap_horizon_site.py <site-slug>-horizon`; this copies the boilerplate and installs dependencies so skeleton work can start immediately.
...
## Phase: Define the Visual System
...
4. **Assemble the skeleton**
   ...
   - Do not edit any copy yet; keep template placeholder text in place until the Draft Page Copy phase.
5. **Log structure choices**
   ...
   - Leave the `## Sections Remaining To Use` list untouched until copy is finalized in the Draft Page Copy phase.

6. **Apply global theming**
   - Implement the palette and typography decisions using the override hooks captured in the Plan the Page Set phase once the first skeleton is stable. Keep the changes scoped to the files listed under `config.yaml > theming`.
...
## Asset Pipeline (Before Site-Wide Assembly)
