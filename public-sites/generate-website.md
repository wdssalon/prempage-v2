...
- `public-sites/scripts/build-static-site.sh <slug>` now copies the Next.js `out/` folder into `public-sites/dist/<slug>/` and drops the manifest beside it as `assets-manifest.json`. The dist folder is the handoff bundle for uploads or CDN sync.
