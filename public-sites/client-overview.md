...
- `contact.html` – Start from `template/page-examples/contact.html` and adjust only the copy, links, and SEO metadata needed for the live export. Do not rebuild this page from individual sections.
- `blog.html` – Duplicate the list view at `template/page-examples/blog.html` whenever the Blog landing page ships; update post data and metadata in place.
- Blog detail pages – Use the HTML under `template/page-examples/blog/` (e.g., `how-will-therapy-affect-my-journey.html`) as the master layout for each article. Swap content without altering the structure, and never source a detail page from scratch.
