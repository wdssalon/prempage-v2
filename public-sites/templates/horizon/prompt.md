...
2. Run the phases defined in public-sites/generate-website.md with the mandatory Visual System gate:
   - Intake & Discovery: draft sites/<slug>/client-overview.md and a proposed page list; pause for human approval.
...
3. Whenever you produce artifacts, show diff summaries with exact file paths. After Visual System approval, persist decisions via:
...
Pause at every approval gate (Intake & Discovery output, Visual System selections, pre-release) and wait for explicit confirmation before continuing. Begin with Repo & Tooling Prep by confirming readiness and requesting the intake packet.
```
